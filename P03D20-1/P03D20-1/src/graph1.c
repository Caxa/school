...
void handle_alpha(char* infix, char* postfix, char* stack, int* top, int* j, int* i) {
    char token[10];
    int k = 0;
...
void infix_to_postfix(char* infix, char* postfix) {
    ...
    for (i = 0; infix[i] != '\0'; i++) {
        ...
        if (is_digit(c) || c == '.') {
            ...
            handle_alpha(infix, postfix, stack, &top, &j, &i);
        } else if (c == '(' || c == ')') {
...
/*int is_function(const char* token) {
    return (strncmp( token,'sin',3)) ||strncmp( token,'cos',3) ||strncmp( token,'ctg',3) ||strncmp( token,'tg',2)||strncmp( token,'sqrt',4);
}*/
...
int main() {
    ...
    /*if (check_invalid_characters(infix)) {
        printf("Ошибка: найдены невалидные символы.\n");
        return 1;
    }*/

    infix_to_postfix(infix, postfix);
    printf("Постфиксная нотация: %s", postfix);
